...
\vspace*{-0.4cm}
\noindent{Due date: September 14, 2017}%\\%\vspace*{0.5cm}
...
\item Write a Python function, namely \texttt{eomTBI} that returns $\dot x$ for given $(t,x)$. This function will be used by the \texttt{scipy.integrate.odeint} integrator.The first few lines and the last line of \texttt{eomTBI} are given as follows:
\begin{verbatim}
def eomTBI(state, t):
    R1 = state[0:2]
    V1 = state[3:5];
    ...
    .
    .
    return np.array([R1_dot, V1_dot, R2_dot, V2_dot])
\end{verbatim}
