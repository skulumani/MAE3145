...
%    Update.m - uses the TOF to calculate the future  COEs
%    RVpqw.m - converts the future COEs into the PQW RV vector
%    RVijk.m - coverts the PQW vectors into IJK system
%
% References:
...
while not(feof(fidin))
    ...
    %Call RVpqw to calculate the RV vectors in the PQW system
    [Rpqw,Vpqw]=RVpqw(af,eccf,nuf);
    
    %Call RVijk to transform the RV vectors into IJK system
    [Rijk,Vijk]=RVijk(Rpqw,Vpqw,inclf,raanf,argpf);
   
    %Call COEprint to print all values to output file
    COEprint(Ri,Vi,af, eccf,inclf,raanf,argpf,nui,nuf,rp,ra,sme,period,tof,Rpqw,Vpqw,Rijk,Vijk,fidout,count);
